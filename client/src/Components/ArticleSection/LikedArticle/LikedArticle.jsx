...
const LikedArticle = () => {
  ...
  useEffect(() => {
      const fetchLikedArticles = async () => {
        try { 
          const url = `${process.env.REACT_APP_API_URL}/likedArticles`;
          ...
          if (response.status === 200) {
            ...
            alert('Failed to fetch liked articles.');
          }
        } catch (error) {
          console.log();
        }
      ...
      <div className='flex flex-col space-y-7 items-center mt-6'>
        {
          likedArticles.length > 0 ? (
            likedArticles.map(article => (
              <div onClick={() => navigateArticleDetails(article._id)} key={article._id} className='bg-slate-800 shadow-md p-4 w-[80%] rounded-md cursor-pointer'>
                <div className='flex'>
